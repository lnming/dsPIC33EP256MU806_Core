...
// Tool suites		: Microchip MPLAB X IDE v3.45 or above
//                	  MPLAB XC16 C-Compiler v1.30 or above
...
// RE0 = Servo motor 6
// RE4 = Servo motor 7
// Note: For 4 output channels only RD0 to RD3 are used.  
//       For 6 output channels only RD0 to RD5 are used.
...
///               1) This routine support up to 6 (SIX) simultaneous PWM outputs, refer to the
...
///                  from 60 MHz to 68 MHz in step of 2 MHz.  Also variable output channels is
///                  now supported.  User can select between 4, 6, and 8 output channels.  For 
///                  instance if 6 output channels is selected, then the output two output pins
...
///               2. Start the driver by setting gnRunPWMServo to 1.  After this is done 
...
void Proce_PWMServo_Driver(TASK_ATTRIBUTE *ptrTask)
{
    ...
    if (ptrTask->nTimer == 0)
    {
        ...
        {
            case 0: // State 0 - Power up.  Wait until unEnPWM is set to 1.
