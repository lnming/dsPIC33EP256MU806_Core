...
// Toolsuites		: Microchip MPLAB X IDE v3.45 or above
...
float       gfVelocityLW = 0.0;         // Velocity of left wheel in rotation/sec.
float       gfVelocityRW = 0.0;         // Velocity of right wheel in rotation/sec.
float       gfVelocityW = 0.0;          // Average velocity of both wheels.
long        gnDistanceMoveLW = 0;       // Distance traveled by left wheel in no. of ticks, 32 bits integer.
long        gnDistanceMoveRW = 0;       // Distance traveled by right wheel in no. of ticks, 32 bits integer.
...
int         gnQuadEncoderStatus = 0;    // 0 - Reset.  
                                        // Otherwise - Ready.
...
unsigned int gunGearRatio = 1;          // Gear ratio for gearbox connected to motor shaft. 
                                        // Note: If we wish to measure actual motor shaft speed, we should
                                        // set this to 1.
unsigned int gunEncoder_CPR = 1;        // Rotational encoder's count pulse per rotation, 
...
unsigned int gunEncoder_Div = 16;       // Quadrature encoder division ratio. Valid value is 1, 2, 4, 8, 16, 32 and 64. 
                                        // We use this parameter to set the no. of ticks per rotation of the wheel. 
                                        // This parameter is used in conjuction with gunGearRatio and gunEncoder_CPR.
...
#define _ENCODER_SCALE_FACTOR   1.0             // Depending on the motor type, gearbox, the actual rotation
                                                ...
                                                // magnitude.
//#define _DT_US                __TCLK_US*64*0.5     // Duration for 1 timer interval, assuming INTDIV bits are set to 0b110.

#define _DT_US                __TCLK_US*256*0.5     // Duration for 1 timer interval, assuming INTDIV bits are set to 0b111.
                                                // Note that the Interval Timer in the Quadrature encoder is incremented
                                                // on both L-to-H and H-to-L transitions of the interval clock, thus we need
                                                // to multiply by 0.5, see documentation of QEI in dsPIC33E.
...
///
///
/// Description		: This driver serves as the interface between user process and the QEI1 and 2 modules
///                   of the dsPIC33E micro-controller.  It is assumed QEI1 is linked to Left Wheel, while
///                   QEI2 is linked to Right Wheel.  
///
///                   Example of Usage:
///                   To use, the user needs to (follow the sequence):
///                   1) Set the global variables gunGearRatio and gunEncoder_CPR for each geared
...
///                   2) Set gnQuadEncoderStatus = _QUADENCODER_RESET.
///                   3) Once gnQuadEncoderStatus = _QUADENCODER_READY, the user routines can then monitor the global 
...
///                   monitoring the global variables again.
///
///                   During program execution, the user can reset this driver and all the global variables by repeating
///                   steps (1) to (3) above.
/// 
/// --- Some notes on fixing the value of _ROTATION_COEF and velocity computation, 31 March 2016 ---
...
///
/// Velocity = _ROTATION_COEF/INT1HLDL;
///                                                
...
/// INTDIV = 256
/// _DT_US = 0.01667*256 usec (assume processor is running at 120 MHz clock, divide by 2 for 1 instruction cycle)
/// _ROTATION_COEF = 195.2734
...
///  Vmax = 195.2734 rotation/sec
/// Min velocity corresponds to Interval Timer = 65535:
///  Vmin = 0.00298 rotation/sec
/// Hence anything less than 0.00298 will be approximated as zero.
///
/// Example 2: Optical quadrature with 20 CPR encoder mounted on the wheel.  Here the gear motor used is not important.
/// _ENCODER_CPR = 20
/// _GEAR_RATIO = 1
/// _ENCODER_SCALE_FACTOR = 0.1
/// INTDIV = 256
/// _DT_US = 0.01667*256 usec
/// _ROTATION_COEF = 1171.6407
/// Max velocity corresponds to Interval Timer = 1: 
///  Vmax = 1171.6407 rotation/sec
/// Min velocity corresponds to Interval Timer = 65535:
///  Vmin = 0.01788 rotation/sec
    ...
    if (ptrTask->nTimer == 0)
    {
        switch (ptrTask->nState)
        {
		    ...
		    // Setup IO pins mode and configure the re-mappable peripheral pins:
                ...
                gnQuadEncoderStatus = _QUADENCODER_POWERUP;   // Indicate quadrature encoder is just powered up.
                ...
                QEI1CONbits.INTDIV = 0b111;         // Set clock source divider ratio, 1:256 pre-scalar value.
                ...
                QEI2CONbits.INTDIV = 0b111;         // 1:256 pre-scalar value.
            ...
            case 1: // State 1 - Wait for user to start the quadrature encoder.
                if (gnQuadEncoderStatus == _QUADENCODER_POWERUP)            // If it is still under power up status.
                {
            ...
            case 2: // State 2 - Reset, initialization of global and static variables and Quadrature Encoder 1 & 2.
                fTemp = gunEncoder_CPR*gunGearRatio*0.000001; 
                ...
                switch(gunEncoder_Div)              // Set the quadrature encoder divider ratio.
                {
                    case 1:
                        unDivide = 0;
                        break;
                ...
                gnQuadEncoderStatus = _QUADENDCODER_READY;   // Indicate quadrature encoder is ready
            ...
            case 3: // State 3 - Measure the quadrature encoder position counter and interval timer, then
                ...
                nlTemp = unTemp2;
                //nlTemp = nlTemp<<16;
                nlTemp = (nlTemp<<2) + unTemp;
                if (nPolarity1 > 0)             // Add in direction of rotation.
                {
                    gfVelocityRW = fRotationCoeff/nlTemp;   // Calculate the rotation speed.
                }
                else
                {
                    gfVelocityRW = -fRotationCoeff/nlTemp;   // Calculate the rotation speed.
                ...
                nlTemp = unTemp2;
                //nlTemp = nlTemp<<16;
                nlTemp = (nlTemp<<16) + unTemp;                
                if (nPolarity2 > 0)             // Add in direction of rotation.
                {
                    gfVelocityLW = fRotationCoeff/nlTemp;   // Calculate the rotation speed.
                }
                else
                {
                    gfVelocityLW = -fRotationCoeff/nlTemp;   // Calculate the rotation speed.
                ...
                gfVelocityW = 0.5*(gfVelocityLW + gfVelocityRW);    // Compute average velocity of wheels.
                ...
                if (gnQuadEncoderStatus == _QUADENCODER_RESET)           // Check status setting, note that this global variable
