...
typedef struct StructProceHC05Driver
{
    ...
    int nEnRFTxPeriodicData;        // Indicate whether to send periodic data packet to remote host.
                                    // 0 or else = Don't transmit data periodically.
...
///
/// From Remote Host:
/// [Dev ID][0x55][Command][Argument1][Argument2]  Packet length = 5 bytes
/// [Dev ID][0xAA][Data1][Data2][Data3]  Packet length = 5 bytes
/// [Dev ID][0xAB][Data1][Data2][Data3]  Packet length = 5 bytes
///
/// Usage:
...
///     void HC05_SendPeriodData(void) - Routine to send back periodic data to remote host (if
///                                          _DEBUG mode is enabled).
...
#define     _COEFFICIENTS_INFO      0x12
...
void Proce_BT_HC_05_CommStack(TASK_ATTRIBUTE *ptrTask)
{
        ...
        switch (ptrTask->nState)
	{
            ...
            case 12: // State 12 - Wait for initial message from host computer to establish SPP link.
                     ...
                     //     0: PC or Tablet.
                     //     Otherwise: Smartphone similar devices
                     // Byte 4: Not used, can set to 0.                  
                if ((gSCIstatus.bRXRDY == 1) && (gbytRXbufptr > 4))     // Check if UART receive any valid data.
                {                                                       // data packet from remote host.
                    if (gSCIstatus.bRXOVF == 0)                         // Make sure no overflow error.
                    {
                        ...
                        {
                            nRFHostType = gbytRXbuffer[3];              // Indicate remote RF host is PC or tablet computer
            ...
            case 13: // State 13 - Reply host computer, note that standard packet consist
                ...
                gbytTXbuffer[4] = 5;                    // Bit length of data set 2, 11-bits.
                gbytTXbuffer[5] = 0;
                gbytTXbuffer[6] = 0;
                gbytTXbuffer[7] = 0;
                gbytTXbuflen = 8;
            ...
            case 50: // State 50 - Periodically send back data to remote host.
                     ...
                     // Byte 8-63: Miscellaneous or image data.

                if (gobjDriverHC05.nEnRFTxPeriodicData > 0)      // Check if transmission of periodic data via RF link is enabled.
                {
                    nCounter++;
                }
                if (nCounter > 20)                  // Check timer for sending periodic data back to remote host.
                                                    // NOTE: 13 Aug 2016
                                                    // This will time out (assuming no other commands is received)
                                                    // 20x2 = 40 ticks, or 40/6 = 6.667 msec (Assume 6 ticks for 1 msec).
                                                    // Coupled with 50 msec to send the data packet of 64 bytes, 
                                                    // in total the transmission of periodic data is every 6.667+50=56.667 msec
                                                    // or 17.647 samples every second.  
                {
                    nCounter = 0;
                    ...
                    if (gSCIstatus.bTXRDY == 0)                 // Check if UART module is busy.
                    {
                        ...
                        gSCIstatus.bTXRDY = 1;	// Initiate TX.
                        OSSetTaskContext(ptrTask, 21, 50*__NUM_SYSTEMTICK_MSEC);    // Next to state = 21, timer = 50 msec.
                    }
                    else
                    {
